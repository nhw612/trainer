...
CLASS_WEIGHTS = [1]
N_EPOCHS = 100
PATCH_SZ = 480   # 패치의 크기, 이미지 크기 * 0.25를 넘으면 안됨(DEEPGLOBE 영상은 1024 크기이므로 256의 크기를 넘으면 안됨), 32의 배수이어야 함, 너무 작으면 업샘플링에 어려움이 있음
BATCH_SIZE = 1
TRAIN_SZ = 300  # 훈련 자료 패치의 갯수
VAL_SZ = 100    # 검증 자료 패치의 갯수, 훈련-검증 비율을 3:1로 잡았으므로 패치의 갯수도 4:1로 계산하는 것이 적절할 듯.
...
if __name__ == '__main__':
    ...
    cnt = 1
    # DEEPGLOBE 이미지 불러오기
    for img_id in file_list_image[1:5]:
    ...
    def train_net():
        ...
        x_train, y_train = get_patches(X_DICT, Y_DICT, n_patches=TRAIN_SZ, sz=PATCH_SZ)
        x_val, y_val = get_patches(X_DICT, Y_DICT, n_patches=VAL_SZ, sz=PATCH_SZ)
        model = get_model()
